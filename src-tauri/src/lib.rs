...
mod packet_guard {
    ...
    const FLAG_DROP: u64 = 2;

    fn dll_path() -> Option<std::path::PathBuf> {
        let exe = std::env::current_exe().ok()?;
        let dir = exe.parent()?;
        let candidates = [
            dir.join("WinDivert.dll"),
            dir.join("binaries").join("WinDivert.dll"),
        ];
    ...
    pub fn start(running: Arc<AtomicBool>) {
        std::thread::spawn(move || unsafe {
            ...
            let module = LoadLibraryA(path_cstr.as_ptr());
            if module.is_null() {
                return;
            }
            let open_raw = GetProcAddress(module, b"WinDivertOpen\0".as_ptr() as *const i8);
            let close_raw = GetProcAddress(module, b"WinDivertClose\0".as_ptr() as *const i8);
            if open_raw.is_null() || close_raw.is_null() {
                FreeLibrary(module);
                return;
            }
            let open_fn: FnOpen = std::mem::transmute(open_raw);
            let close_fn: FnClose = std::mem::transmute(close_raw);

            let filter = CString::new(concat!(
                "(tcp.Rst or tcp.Fin) and inbound and ",
                "(ip.Id == 0x0000 or ip.Id == 0x0001 or ip.Id == 0x0004)"
            ))
            .unwrap();

            let handle = open_fn(filter.as_ptr(), LAYER_NETWORK, 0, FLAG_DROP);
...
#[cfg(target_os = "windows")]
fn packet_guard_state() -> &'static Mutex<Option<Arc<AtomicBool>>> {
    static STATE: OnceLock<Mutex<Option<Arc<AtomicBool>>>> = OnceLock::new();
    STATE.get_or_init(|| Mutex::new(None))
}

#[cfg(target_os = "windows")]
fn activate_packet_guard() {
    let mut g = match packet_guard_state().lock() {
        Ok(g) => g,
        Err(e) => e.into_inner(),
    };
    if g.as_ref().map(|r| r.load(Ordering::Relaxed)).unwrap_or(false) {
        return;
    }
    let running = Arc::new(AtomicBool::new(true));
    packet_guard::start(Arc::clone(&running));
    *g = Some(running);
}

#[cfg(target_os = "windows")]
fn deactivate_packet_guard() {
    let mut g = match packet_guard_state().lock() {
        Ok(g) => g,
        Err(e) => e.into_inner(),
    };
